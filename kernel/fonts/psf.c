...
static inline bool is_psf1(const union psf_header * font)
{
	return font->psf1.magic == PSF1_MAGIC;
}

static inline bool is_psf2(const union psf_header * font)
{
	return font->psf2.magic == PSF2_MAGIC;
}

static uint16_t * psf1_unicode_seq(const struct psf1_header * font, uint32_t uc)
{
	size_t nb_glyphs = (font->mode & PSF1_MODE512) ? 512 : 256;
	uint8_t * start = ((uint8_t *) font) + sizeof(*font)
		+ nb_glyphs * font->charsize;
	uint8_t * eof = ((uint8_t *) font) + size;

	while (start < eof && *start != uc) {
		if (*(start++) == PSF1_STARTSEQ && start < eof)
			while (*(start++) != PSF1_SEPARATOR && start < eof);
	}

	while (++start < eof && *start != PSF1_STARTSEQ
	       && *start != PSF1_SEPARATOR);
...
static int psf1_set_glyph(bool * dst, const struct psf1_header * font,
			  uint32_t glyph)
{
	uint8_t * bitmap = ((uint8_t *) font) + sizeof(*font)
		+ glyph * font->charsize;

	for (size_t bit = 0; bit < 8 * font->charsize; bit++) {
		if (bitmap[bit / 8] & (1 << (7 - bit % 8)))
			dst[bit] = true;
	}
}

static int psf1_bitmap(bool * dst, const struct psf1_header * font,
		       uint32_t uc)
{
	int ret;

	if (font->mode & PSF1_MODEHASTAB) {
		uint16_t * seq = psf1_unicode_seq(font, uc);
		if (seq == NULL)
			return -ENOENT;
		if (*seq == PSF1_SEPARATOR) {
			if ((ret = psf1_set_glyph(dst, font, uc)))
				return ret;
		} else if (*seq == PSF1_STARTSEQ) {
			while (*(++seq) != PSF1_SEPARATOR) {
				if ((ret = psf1_set_glyph(dst, font, *seq)))
					return ret;
			}
		}
	} else {
...
static inline size_t psf_header_size(const struct font_psf_font * font) {
	if (font->version == PSF_VERSION_1)
		return sizeof(*(font->header->psf1));
	else if (font->version == PSF_VERSION_2)
		return font->header->psf2.headersize;
	else
		return 0;
}

static inline size_t psf_glyph_size(const struct psf_font * font) {
	if (font->version == PSF_VERSION_1)
		return font->header->psf1.charsize;
	else if (font->version == PSF_VERSION_2)
		return font->header->psf2.charsize;
	else
		return 0;
}
...
static inline bool psf_has_unicode(const struct psf_font * font)
{
	if (font->version == PSF_VERSION_1)
		...
		//return (font->header->psf1.mode & PSF1_MODEHASTAB) != 0;
	else if (font->version == PSF_VERSION_2)
		return font->header->psf2.has_unicode_table;
		//return (font->header->psf2.flags & PSF2_HAS_UNICODE_TABLE) != 0;
	else
		return 0;
}
					    ...
					    uint32_t uc)
{
	...
	if (font->version == PSF_VERSION_1) {
		...
		//if (font->header->psf1.mode & PSF1_MODE512)
