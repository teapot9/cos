...
//void parse(pos, dst, written, fmt)
static void parse(char * dst, const char * fmt, size_t * pos, size_t * written, va_list * ap, size_t len, size_t precision_len, size_t precision)
{
...
{
	// TODO: reduce massive stack usage here
	size_t written = 0;
	size_t pos = 0;
	...
	while (written < len - 1 && fmt[pos]) {
		...
		//parse(dst, fmt, &pos, &written, &ap, &len, &precision_len, &precision);
